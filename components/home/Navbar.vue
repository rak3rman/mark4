        ...
        <div class="rounded-lg shadow-md bg-primary-focus ring-1 ring-black ring-opacity-5 overflow-hidden">
          ...
          <div class="px-2 pt-2 pb-3">
            <PopoverButton v-for="(item, index) in navigation" class="block w-full text-left">
              <a :key="item.name" :href="item.href" :class="'delay-[' + index*100 + ']'" class="block px-3 py-2 rounded-md text-base font-mono text-accent hover:text-secondary hover:bg-primary"><span class="text-secondary">{{ '0' + (index + 1) + '. ' }}</span> {{ item.name }}</a>
            </PopoverButton>
          </div>
