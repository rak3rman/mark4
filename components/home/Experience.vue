    ...
    <div>
        ...
        <div class="mt-4 w-full pb-1">
            <div class="sm:hidden fade-in">
                ...
                <select id="tabs" v-model="selectedExpTab" name="tabs"
                        class="block w-full bg-primary focus:ring-secondary focus:border-secondary border-neutral text-neutral rounded-md">
                    <option v-for="tab in expTabs" :value="tab.id" :selected="tab.id === selectedExpTab"
                            @select="updateExpTab(tab.id)">{{ tab.name }}
                    </option>
                </select>
            </div>
            <div class="hidden sm:block sm:mt-4 sm:mb-3">
                <div class="grid grid-cols-3 gap-x-8" role="tablist" aria-orientation="horizontal">
                    <NuxtLink class="relative transition ease-in-out duration-300 fade-in" v-for="tab in expTabs"
                              :key="tab.id" @click="updateExpTab(tab.id)"
                              :class="[tab.id === selectedExpTab ? '' : 'opacity-75 hover:opacity-100']"
                              to="#experience">
                        <div class="w-9 rounded-lg transition ease-in-out duration-300"
                             :class="[tab.id === selectedExpTab ? 'bg-secondary' : 'bg-primary-focus']">
                            <svg aria-hidden="true" class="h-9 w-9 p-2" fill="#fff" :viewBox="tab.viewbox"
                                 v-html="tab.icon"></svg>
                        </div>
                        <h6 class="mt-6 text-md font-medium font-mono transition ease-in-out duration-300"
                            :class="[tab.id === selectedExpTab ? 'text-secondary' : 'text-neutral']">
                            <button>
            ...
            <div v-for="(entry, index) in expEntries" class="fade-in">
                <div v-if="index < 5 || !limitExp" class="pt-2">
                    ...
                    <h6 class="leading-normal text-neutral text-sm font-mono text-left">
                        {{ gen_date_range(entry.start, entry.end) }} // {{ entry.location }}
                    </h6>
                    ...
                    <p class="text-neutral font-light text-sm text-left pb-0" v-if="entry.bullets.length === 0">
    ...
    [
        {
            ...
            quote: "Radison is not your typical web developer. He is experienced and knowledgeable enough to take your website ideas and turn them into a fast, attractive, and extremely functional online site, all at an affordable price.",
...
const gen_date_range = (start, end) => {
    // Create start date object
    let startDate = DateTime.fromMillis(Date.parse(start)).plus({ months: 1 })
    // Catch edge of future experience (start in future)
    if (startDate > DateTime.now()) {
        return startDate.toFormat('LLL yyyy')
    }
    return startDate.toFormat('LLL yyyy') + ' - ' + (end === 'inf' ? 'Present' : DateTime.fromMillis(Date.parse(end)).plus({ months: 1 }).toFormat('LLL yyyy'))
}
</script>
...
const exp_tabs = [
    {
        ...
        desc: 'Pushing the boundaries of full-stack web dev, cloud computing, dev-ops, and cybersecurity.',
    ...
    },
    {
        id: 'photo',
        name: 'Photography',
        desc: 'Specializing in stills photography of events, sports, music, and landscapes.',
